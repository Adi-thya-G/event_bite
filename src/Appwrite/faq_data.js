...
import { Client, ID, Databases,  Query,Storage,Functions ,ExecutionMethod} from "appwrite";
...
async CREATE_FAQ({question,answer,type})
{
 try
 {
     return await this.databases.createDocument(conf.appwriteDatabaseId,conf.appwriteFaqCollectionId,ID.unique(),
        {
        question:question,
        answer:answer,
        type:type
...
{
    try{
        return await this.databases.deleteDocument(conf.appwriteDatabaseId,conf.appwriteFaqCollectionId,id)
