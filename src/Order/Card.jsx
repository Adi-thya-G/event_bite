import React, { useDebugValue } from "react";
import { useState } from "react";
import Wish from "./Wish";
import { FaRegHeart } from "react-icons/fa";
import Search from "./Search";
import Rating from "./Rating";
import { useSelector,useDispatch } from "react-redux";

function Card({ src, alt, description, name, rating }) {


  const [cart, setcart] = useState(true);
  const [message, setmessage] = useState("");
  
  const handlesubmit = () => {
    clearTimeout(timeout);
    console.log(cart, message);
    if (cart && message == "") {
      setmessage("Added to cart");
    } else {
      setmessage("Removed from cart");
    }
    setTimeout(timeout, 1000);

    console.log(cart, "after");
  };

  // set time out function
  const timeout = () => {
    setmessage("");
    setcart((pre) => !pre);
  };
  return (
    <>
      <div className=" mb-40 w-[490px] h-[200px] bg-zinc-50 border-2 border-zinc-100 pl-1 rounded-sm   flex max-md:w-[320px]">
        <div className="w-2/5 h-[200px] flex items-center ">
          <img className=" w-full object-cover h-[90%]" src={src} alt="" />
        </div>
        <div className="w-[280px] h-[160px] bg-stone-50 my-auto ml-2">
          <div className="h-5/6 mt-1">
            <div className="flex justify-end">
              <Rating rating={rating} />
            </div>
            <h2 className="flex justify-center text-[25px] text-orange-400 font-semibold font-mono py-2">
              {" "}
              {name}
            </h2>
            <p className="text-[15px]">
              Lorem ipsum, dolor sit amet consectetur adipisicing elit. Vel. .
            </p>
            <div></div>
          </div>
          <div className="h-1/6 w-[100%] flex ">
            <Wish id="one"/>
            {/*message*/}
            <p className={`${cart ? "text-green-300" : "text-red-400 "} ml-9`}>
              {message}
            </p>

            <button
              className="w-8 h-8 p-1 ml-auto rounded-md"
              onClick={() => handlesubmit()}
            >
              <svg
                fill="#000000"
                version="1.1"
                id="Capa_1"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                width="28px"
                height="28px"
                viewBox="0 0 902.86 902.86"
                xml:space="preserve"
              >
                <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
                <g
                  id="SVGRepo_tracerCarrier"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></g>
                <g id="SVGRepo_iconCarrier">
                  {" "}
                  <g>
                    {" "}
                    <g>
                      {" "}
                      <path d="M671.504,577.829l110.485-432.609H902.86v-68H729.174L703.128,179.2L0,178.697l74.753,399.129h596.751V577.829z M685.766,247.188l-67.077,262.64H131.199L81.928,246.756L685.766,247.188z"></path>{" "}
                      <path d="M578.418,825.641c59.961,0,108.743-48.783,108.743-108.744s-48.782-108.742-108.743-108.742H168.717 c-59.961,0-108.744,48.781-108.744,108.742s48.782,108.744,108.744,108.744c59.962,0,108.743-48.783,108.743-108.744 c0-14.4-2.821-28.152-7.927-40.742h208.069c-5.107,12.59-7.928,26.342-7.928,40.742 C469.675,776.858,518.457,825.641,578.418,825.641z M209.46,716.897c0,22.467-18.277,40.744-40.743,40.744 c-22.466,0-40.744-18.277-40.744-40.744c0-22.465,18.277-40.742,40.744-40.742C191.183,676.155,209.46,694.432,209.46,716.897z M619.162,716.897c0,22.467-18.277,40.744-40.743,40.744s-40.743-18.277-40.743-40.744c0-22.465,18.277-40.742,40.743-40.742 S619.162,694.432,619.162,716.897z"></path>{" "}
                    </g>{" "}
                  </g>{" "}
                </g>
              </svg>
            </button>
          </div>
        </div>
      </div>
    </>
  );
}

export default Card;
